...
def snowflake_permission_sql(
    *,
    ...
    lines = [
        ...
        "",
        "-- Source snapshot pinning (Snowflake Time Travel on physical tables).",
        "-- Views are expanded to leaves; SELECT on views is required to read VIEW_DEFINITION.",
        "-- Time Travel requires DATA_RETENTION_TIME_IN_DAYS > 0 on those tables.",
        "-- Also grant SELECT on seed / source schemas those views read.",
        f"-- GRANT USAGE ON SCHEMA {database}.{schema}_RAW TO ROLE {role};",
        f"-- GRANT SELECT ON ALL TABLES IN SCHEMA {database}.{schema}_RAW TO ROLE {role};",
        "",
